...
export function Combobox({
  ...
  return (
    ...
    >
      <span
        data-slot="control"
        className={clsx(className, TRIGGER_FRAME)}
      >
        {TriggerIcon && (
          <span
            className="pointer-events-none absolute inset-y-0 left-0 z-10 flex items-center pl-3 text-zinc-500 dark:text-zinc-400 sm:pl-2.5"
            aria-hidden
          >
            <TriggerIcon className="h-4 w-4 sm:h-3.5 sm:w-3.5" />
          </span>
        )}
        ...
        >
          <ChevronDown
            className="h-5 w-5 stroke-zinc-500 group-data-disabled:stroke-zinc-600 group-data-hover:stroke-zinc-700 sm:h-4 sm:w-4 dark:stroke-zinc-400 dark:group-data-hover:stroke-zinc-300"
            aria-hidden
          />
        </Headless.ComboboxButton>
      </span>
